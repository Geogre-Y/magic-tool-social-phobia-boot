...
public class LoginController {
	...
	@ApiOperation("WX跳转链接登录接口")
	@RequestMapping(value = "/wxLogin", method = RequestMethod.POST)
	public Result<JSONObject> wxLogin(@RequestBody Map<String,String> data){
		Result<JSONObject> result = new Result<JSONObject>();
		String openId = data.get("url");
		String phone = data.get("phone");
		if (StrUtil.isEmpty(openId)){
			return Result.error("路径错误");
		}
		QueryWrapper<SysUser> q1 = new QueryWrapper<>();
		q1.eq("wx_open_id",openId);
		SysUser sysUser = sysUserService.getOne(q1);
		if (ObjectUtil.isNull(sysUser)){
			log.info("该用户未注册，自动注册中。。。。");
			if (StrUtil.isEmpty(phone)){
				return Result.error("请填写手机号");
			}
			defaultRegister(openId,phone);
		}else {
			log.info("该用户已注册，正在登录中。。。。");
		}
		//1. 校验用户是否有效
		SysUser loginUser = sysUserService.getOne(q1);
		//update-end-author:wangshuai date:20200601 for: 登录代码验证用户是否注销bug，if条件永远为false
		result = sysUserService.checkUserIsEffective(loginUser);
		if(!result.isSuccess()) {
			return result;
		}
		//用户登录信息
		...
		BeanUtils.copyProperties(loginUser, returnLoginUser);
		baseCommonService.addLog("用户名: " + loginUser.getUsername() + ",登录成功！", CommonConstant.LOG_TYPE_1, null,returnLoginUser);
        //update-end--Author:wangshuai  Date:20200714  for：登录日志没有记录人员
		return result;
	}

	public void defaultRegister(String openId, String phone) {
		SysUser user = new SysUser();
		user.setCreateTime(new Date());// 设置创建时间
		String salt = oConvertUtils.randomGen(8);
		String name = "机器人" + phone;
		String passwordEncode = PasswordUtil.encrypt(name, "123456", salt);
		user.setSalt(salt);
		...
		user.setAvatar("https://tse3-mm.cn.bing.net/th/id/OIP-C.M2FiQTC9Nc5YyYfd7qtf8gAAAA?rs=1&pid=ImgDetMain");
	...
	public Result<String> randomImage(HttpServletResponse response,@PathVariable("key") String key){
		...
		try {
			...
			redisUtil.set(realKey, lowerCaseCode, 360);
